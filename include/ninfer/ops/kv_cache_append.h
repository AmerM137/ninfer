 ...
 * D=128, Hkv=8 and capacity 2048 or 4096. Absolute position p maps to slot p mod capacity. The
 * caller guarantees that each row's existing live interval ends immediately before
 * positions[0,b], advancing it by counts[b] makes every overwritten old slot dead, and one row
 * commits at most the ring capacity.
