...
int run_w8_qkv_case(DevicePackedWeight& parent, std::int32_t hidden, const char* profile,
    ...
    for (int phase = 0; phase < (graph_replay ? 2 : 1); ++phase) {
        if (phase) {
            ...
            activation_bits = bf16_bits(activation);
            // The replay stream is nonblocking, so a default-stream DeviceBuffer copy is not
            // ordered before the graph launch. Keep the phase input update on the captured
            // stream so every replay observes its represented activation.
            cuda_check(cudaMemcpyAsync(device_activation.p, activation_bits.data(),
                                       device_activation.bytes, cudaMemcpyHostToDevice, stream),
                       "refresh graph-replay activation");
        }
        cuda_check(
