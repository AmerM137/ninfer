...
BuiltResponse build_response(const std::string& id, std::int64_t created_at,
                             const ResponsesRequest& request, const ResponsesRuntimeValues& runtime,
                             const GenerationOutcome& outcome, ResponsesItemIds ids) {
    BuiltResponse built;
...
Json ResponsesEventStream::event(std::string type, Json fields) {
    fields["type"]            = std::move(type);
    fields["sequence_number"] = sequence++;
    return fields;
}

std::vector<std::string> ResponsesEventStream::ensure_reasoning() {
...
std::vector<std::string> ResponsesEventStream::close_reasoning(const std::string& final_text,
...
std::vector<std::string> ResponsesEventStream::ensure_message() {
...
std::vector<std::string> ResponsesEventStream::close_message(const std::string& final_text,
                                                             const char* item_status) {
...
ResponsesEventStream::ResponsesEventStream(std::string response_id, std::int64_t created_at,
    ...
    : id(std::move(response_id)), created_at(created_at), request(std::move(request)),
      runtime(runtime) {}

std::vector<std::string> ResponsesEventStream::start() {
    if (started) { throw std::logic_error("Responses event stream already started"); }
    started             = true;
    const Json response = in_progress_response(id, created_at, request, runtime);
    return {sse(event("response.created", Json{{"response", response}})),
            sse(event("response.in_progress", Json{{"response", response}}))};
}

std::vector<std::string> ResponsesEventStream::reasoning_delta(const std::string& text) {
    if (!started || finish_built) { throw std::logic_error("invalid reasoning delta event state"); }
    if (text.empty()) { return {}; }
    std::vector<std::string> events = ensure_reasoning();
    reasoning_text += text;
    events.push_back(
        sse(event("response.reasoning_text.delta", Json{{"item_id", ids.reasoning},
                                                        {"output_index", reasoning_index},
                                                        {"content_index", 0},
                                                        {"delta", text}})));
    return events;
...
std::vector<std::string> ResponsesEventStream::content_delta(const std::string& text) {
    if (!started || finish_built) { throw std::logic_error("invalid content delta event state"); }
    if (text.empty()) { return {}; }
    std::vector<std::string> events = close_reasoning(reasoning_text);
    std::vector<std::string> added  = ensure_message();
    events.insert(events.end(), std::make_move_iterator(added.begin()),
                  std::make_move_iterator(added.end()));
    content_text += text;
    events.push_back(sse(event("response.output_text.delta", Json{{"item_id", ids.message},
                                                                  {"output_index", message_index},
                                                                  {"content_index", 0},
                                                                  {"delta", text},
                                                                  {"logprobs", Json::array()}})));
...
ResponsesStreamFinish ResponsesEventStream::finish(const GenerationOutcome& outcome) {
    ...
    if (!outcome.reasoning.empty() && !reasoning_started) {
        append(ensure_reasoning());
    ...
    append(close_reasoning(outcome.reasoning, reasoning_status));

    const bool needs_message = !outcome.text.empty() || outcome.tool_calls.empty();
    if (needs_message) {
        append(ensure_message());
        if (outcome.text != content_text) {
            if (!outcome.text.starts_with(content_text)) {
                throw std::logic_error("streamed content does not match terminal content");
            }
            const std::string suffix = outcome.text.substr(content_text.size());
            if (!suffix.empty()) {
                content_text += suffix;
                finished.events_before_terminal.push_back(
                    sse(event("response.output_text.delta", Json{{"item_id", ids.message},
                                                                 {"output_index", message_index},
                                                                 {"content_index", 0},
                                                                 {"delta", suffix},
                                                                 {"logprobs", Json::array()}})));
        ...
        append(close_message(outcome.text, item_status));
    }

    ids.function_calls.reserve(outcome.tool_calls.size());
    for (const ToolCall& call : outcome.tool_calls) {
        const std::string item_id = new_response_item_id("fc");
        ids.function_calls.push_back(item_id);
        ...
        if (!call.arguments_json.empty()) {
            finished.events_before_terminal.push_back(sse(event(
    ...
    finished.response = build_response(id, created_at, request, runtime, outcome, ids);
    return finished;
}

std::string ResponsesEventStream::terminal(const BuiltResponse& response) {
    if (!finish_built || terminal_emitted) {
    ...
    const std::string type   = status == "completed"    ? "response.completed"
                               ...
                               : status == "cancelled"  ? "response.cancelled"
                                                        : "response.failed";
    return sse(event(type, Json{{"response", response.body}}));
}

std::string ResponsesEventStream::failed(const ApiError& error) {
    if (!started || terminal_emitted) {
        throw std::logic_error("invalid Responses failed event state");
    }
    terminal_emitted         = true;
    Json response            = in_progress_response(id, created_at, request, runtime);
    response["status"]       = "failed";
    response["completed_at"] = completion_time_now();
    response["error"]        = Json{{"code", error.code.empty() ? Json(nullptr) : Json(error.code)},
                                    {"message", error.message}};
    return sse(event("response.failed", Json{{"response", response}}));
}
