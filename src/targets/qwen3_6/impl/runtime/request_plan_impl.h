...
ProgramImplCore::plan_request_base(const PreparedPromptData& prompt,
    ...
    const std::size_t cold_prefill_splits =
        ...
        (base.rewrite_checkpoint && base.rewrite_checkpoint->frontier < base.summary.prompt_tokens
             ? 1ULL
             : 0ULL);
...
RequestPlan ProgramImplCore::plan_request_for_lane(std::uint32_t lane,
    ...
    RequestPlan plan;
    plan.summary                     = base_plan.summary;
    plan.sampling                    = base_plan.sampling;
    plan.text_kv_page_entitlement    = base_plan.text_kv_page_entitlement;
    plan.backend_kv_page_entitlement = base_plan.backend_kv_page_entitlement;

    if (base_plan.allow_prefix_reuse && prompt.identity.reusable && sequence.retained) {
    ...
    const std::optional<RewriteCheckpointSpec>& desired = base_plan.rewrite_checkpoint;
    ...
    if (base_plan.vision_control != nullptr) {
        VisionPrefillPlan vision;
        vision.control = base_plan.vision_control;
        vision.uses.reserve(base_plan.vision_control->items.size());
        for (std::size_t index = 0; index < base_plan.vision_control->items.size(); ++index) {
            const qwen3_6::VisionItemControl& item = base_plan.vision_control->items[index];
            const auto first          = static_cast<std::uint32_t>(item.scatter_indices.front());
            const auto last           = static_cast<std::uint32_t>(item.scatter_indices.back());
        ...
        if (!vision.uses.empty()) {
            ...
            plan.summary.transient_bytes     = base_plan.vision_transient_bytes;
